...
void writeIntensityField2(Mesh* pm, int file_number) {
    Real r, theta, phi, innerRadius, outerRadius;
    int rank, size, finalFile;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    finalFile=7;
    int numBlocks = pm->nblocal;
    std::string fileName1 = "intensities." + std::to_string(file_number) + ".txt";
    std::string fileName2 = "locations." + std::to_string(file_number) + ".txt";
    std::string restartFileIntensity="intensitiesRestart.txt";
    std::string restartFileLocation="locationsRestart.txt";
    Real innerRadiusStart = 1.7 * std::pow(10.0, 7.0);
    //Real innerRadiusStart=6.236*std::pow(10.0,7.0);
    Real deltaR=.006*std::pow(10.0,8.0);
    
    // Buffers for local data
    std::ostringstream localRadiationData;
    std::ostringstream localLocationData;
    std::ostringstream restartRadiationData;
    std::ostringstream restartLocationData;
    Real t= pm->time;
    auto pmb= pm->my_blocks(0);



    Real vmax=2100.0;

    innerRadius = innerRadiusStart + vmax*t;
    outerRadius=innerRadius+deltaR;
    int seen=0;
        ...
        for (int k = pmb->ks; k <= pmb->ke; ++k) {
            for (int j = pmb->js; j <= pmb->je; ++j) {
                ...
                for (int i = pmb->is; i <= pmb->ie; ++i) {
                    double r = pmb->pcoord->x1v(i);
                    double theta = pmb->pcoord->x2v(j);
                    double phi = pmb->pcoord->x3v(k);
                    if (file_number==finalFile) {
                      std::ostringstream lineRadiation;
                      lineRadiation.str("");  // Reset the content of the stream (optional)
                      lineRadiation.clear(); 
                      for (int ifr = 0; ifr < pmb->pnrrad->nfreq; ifr++) {
                          for (int n = 0; n < nang; n++) {
                              double intensity = (&(pmb->pnrrad->ir(k, j, i, ifr * nang)))[n];
                              lineRadiation << intensity << " ";
                          }
                      }
                      lineRadiation << "\n";
                      std::ostringstream lineLocation;
                      lineLocation.str("");  // Reset the content of the stream (optional)
                      lineLocation.clear(); 
                      lineLocation << r << " " << theta << " " << phi << "\n";
                      
                      for(int writer=0; writer<size; writer++) {

                          if(rank==writer) {
                            std::ofstream restartRadiationFile(restartFileIntensity, std::ios::app);
                            std::ofstream restartLocationFile(restartFileLocation, std::ios::app);

                            if (restartRadiationFile.is_open() && restartLocationFile.is_open()) {
                                restartRadiationFile.write(lineRadiation.str().c_str(), lineRadiation.str().size());
                                restartLocationFile.write(lineLocation.str().c_str(), lineLocation.str().size());
                            }

                            restartRadiationFile.close();
                            restartLocationFile.close();
                          }
                          MPI_Barrier(MPI_COMM_WORLD);
                      }

                    }
                    if (r > innerRadius && r < outerRadius && seen==0) {
                      seen=1;
                      for (int ifr = 0; ifr < pmb->pnrrad->nfreq; ifr++) {
                          for (int n = 0; n < nang; n++) {
                              Real intensity = (&(pmb->pnrrad->ir(k, j, i, ifr * nang)))[n];
                              localRadiationData << intensity << " ";
                          }
                      }
                      localRadiationData << "\n";
                      localLocationData << r << " " << theta << " " << phi << "\n";
                    }
                }
            }
        }
    ...
    // Convert local data to strings
    std::string localRadiationStr = localRadiationData.str();
    std::string localLocationStr = localLocationData.str();
  
    // Gather lengths of local strings
    int localRadiationSize = localRadiationStr.size();
    int localLocationSize = localLocationStr.size();
    std::vector<int> radiationSizes(size);
    std::vector<int> locationSizes(size);

    MPI_Gather(&localRadiationSize, 1, MPI_INT, radiationSizes.data(), 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Gather(&localLocationSize, 1, MPI_INT, locationSizes.data(), 1, MPI_INT, 0, MPI_COMM_WORLD);

    // Compute displacements
    std::vector<int> radiationDispls(size, 0);
    std::vector<int> locationDispls(size, 0);
    int totalRadiationSize = 0, totalLocationSize = 0;
    for (int i = 0; i < size; ++i) {
        if (i > 0) {
            radiationDispls[i] = radiationDispls[i - 1] + radiationSizes[i - 1];
            locationDispls[i] = locationDispls[i - 1] + locationSizes[i - 1];
        }
        totalRadiationSize += radiationSizes[i];
        totalLocationSize += locationSizes[i];
    }

    // Gather all data to the root process
    std::vector<char> allRadiationData(totalRadiationSize);
    std::vector<char> allLocationData(totalLocationSize);

    MPI_Gatherv(localRadiationStr.c_str(), localRadiationSize, MPI_CHAR,
        allRadiationData.data(), radiationSizes.data(), radiationDispls.data(), MPI_CHAR, 0, MPI_COMM_WORLD);
    MPI_Gatherv(localLocationStr.c_str(), localLocationSize, MPI_CHAR,
        allLocationData.data(), locationSizes.data(), locationDispls.data(), MPI_CHAR, 0, MPI_COMM_WORLD);

    // Write to files from the root process
    if (rank == 0) {
        std::ofstream radiationFile(fileName1, std::ios::app);
        std::ofstream locationFile(fileName2, std::ios::app);

        radiationFile.write(allRadiationData.data(), totalRadiationSize);
        locationFile.write(allLocationData.data(), totalLocationSize);
...
void writeIntensityFieldCartesian(Mesh* pm, int file_number) {
    Real r, theta, phi, innerRadius, outerRadius;
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    ...
    innerRadius = 1.797 * std::pow(10.0, 8.0);
    outerRadius = 1.803 * std::pow(10.0, 8.0);
    ...
    for (int b = 0; b < numBlocks; b++) {
        ...
        for (int k = pmb->ks; k <= pmb->ke; ++k) {
            for (int j = pmb->js; j <= pmb->je; ++j) {
                for (int i = pmb->is; i <= pmb->ie; ++i) {
                    auto x = pmb->pcoord->x1v(i);
                    auto y = pmb->pcoord->x2v(j);
                    auto z = pmb->pcoord->x3v(k);
                    Real r=(x*x+y*y+z*z);
...
void ATHDF5Output::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool flag) {
  ...
  writeIntensityField2(pm, output_params.file_number);
